...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector,activeClass) {
    ...
    tabsParent.addEventListener('click', (event) => {
        ...
        if (target && target.classList.contains(tabsSelector.slice(1))) { // в селекторе отрезаем точку (первый символ)
            tabs.forEach((item, i) => {
                if (target == item) {
                    hideTabContent();
                    showTabContent(i);
...
window.addEventListener('DOMContentLoaded', () => {  
    ...
    (0,_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '2021-10-17T12:00:00');
